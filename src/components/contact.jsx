export default function Contact(props){
        ...
        <article id={props.id} onClick={props.onClick} className={`rounded-lg p-2 w-full relative hover:cursor-pointer max-w-[300px] h-[75px] ${props.active?"bg-primary":"bg-inherit"}`}>
            <div className="flex h-full gap-2">
                ...
                <img src={props.profilePic} alt="img" className="h-full w-full object-contain"/>
                ...
                <div>
                    ...
                    <p className={`text-xs text-justify text-light-text ${props.active?"text-white":"text-light-text"}`}>{props.lastText?props.lastText.length>81?props.lastText.substring(0,77)+" ...":props.lastText:"No recent messages"}</p>
                </div>
            </div>
        </article>
    )
