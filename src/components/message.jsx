    ...
    <div className={`messgae-wrapper w-full`}>
        <div className={`message border-0 float p-1 flex items-center ${type=="primary"?"":"flex-row-reverse"}`}>
            <div className="flex flex-col">
                <img src={props.img} alt="" className={`${type=="primary"?"bg-gray-600":"bg-slate-400"} rounded-full w-[30px] h-[30px] border-2 border-green-200`} />
...
const GroupMessage = (props)=>{
    ...
    return(
        ...
        <div className={`group-message border-0 float p-2 flex items-center ${type=="primary"?"":"flex-row-reverse"}`}>
            ...
            <div>
                <p className={`message-header px-6 space-x-1`}>
                    <span className="text-white">{props.userName}</span>
                    <span className="text-xs text-light-text">{props.messageTime}</span>
                </p>
                <p className={`message-text text-justify px-5 py-3 ${type=="primary"?"primary bg-primary":"secondary bg-secondary"} mx-4 rounded-2xl shadow-md before:absolute relative max-w-[300px] text-cream`}>{props.text}</p>
            </div>
        </div>
        </div>
    )
